async function handler(m, { conn, text }) {
  const fs = await import('fs').then(x => x.default || x)
  const path = await import('path').then(x => x.default || x)
  ...
  // Deduplicazione
  const cacheKey = `demote_${m.chat}_${user}`
  if (global.promoteDemoteCache?.has(cacheKey)) {
    return conn.sendMessage(m.chat, { text: 'ⓘ Retrocessione in corso...' }, { quoted: m })
  }
  ...
  const text2 = `📉 @${userPhone} è stato retrocesso da @${senderPhone}`
    const retrocessione = `Retrocessione`

  const __dirname = path.dirname(fileURLToPath(import.meta.url))
  const fallbackPath = path.join(__dirname, '../../media/fallback.png')
  const messageOptions = { mentions: [user, m.sender] }
