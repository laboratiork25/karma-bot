 function ensureWarnState(user, chatId) {
    ...
    if (!(chatId in user.warns) && Number(user.warn) > 0) user.warns[chatId] = Number(user.warn);
...
let handler = async (m, { conn, args, groupMetadata }) => {
    ...
    let warn = Number(user.warns[m.chat] || 0);
...
handler.help = [
  'delwarn @user',
  'unwarn @user',
  'rimuoviavvertimento @user',
  'quitaraviso @usuario',
  'removeraviso @usuário',
  'warnlöschen @benutzer',
  '删除警告 @用户',
  'удалитьпредупреждение @пользователь',
  'إزالةتحذير @مستخدم',
  'चेतावनीहटाओ @उपयोगकर्ता',
  'supprimeravertissement @utilisateur',
  'hapusperingatan @pengguna',
  'uyarısil @kullanıcı'
];
handler.tags = ['group'];
