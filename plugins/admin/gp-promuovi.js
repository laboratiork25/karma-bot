...
const getParticipantPhoneJid = (participant, conn) => {
    for (const candidate of [
        ...
        const normalized =
            normalizePhoneJid(candidate.trim()) ||
            conn.decodeJid(candidate)
...
const resolveTarget = (jid, participants, conn) => {
    ...
    const match = participants.find(participant => {
        ...
        return values.some(value => {
            ...
            const normalized =
                normalizePhoneJid(value.trim()) ||
                conn.decodeJid(value)

            return (
                normalized === decoded ||
                getJidUser(normalized) === jidUser
            )
        })
    })

    return match
        ? (getParticipantPhoneJid(match, conn) || conn.decodeJid(match.id))
        : decoded
}
...
async function handler(m, { conn, text }) {
    ...
    const senderParticipant = participants.find(participant => {
        const participantId = conn.decodeJid(participant.id)
        const phoneJid = getParticipantPhoneJid(participant, conn)

        return (
            participantId === sender ||
            phoneJid === sender ||
            getJidUser(participantId) === getJidUser(sender)
        )
    })
    ...
    if (
        ...
        return conn.sendMessage(
            m.chat,
            {
                text: 'ⓘ 𝒮𝑜𝓁𝑜 𝑔𝓁𝒾 𝒶𝒹𝓂𝒾𝓃 𝓅𝑜𝓈𝓈𝑜𝓃𝑜 𝓊𝓈𝒶𝓇𝑒 𝒾𝓁 𝒸𝑜𝓂𝒶𝓃𝒹𝑜.'
            },
            { quoted: m }
        )
    }
    ...
    const mentionTarget =
        m.mentionedJid?.[0] ||
        rawMentions
            .map(jid => resolveTarget(jid, participants, conn))
            .find(Boolean)

    const quotedTarget = m.quoted?.sender
        ? resolveTarget(m.quoted.sender, participants, conn)
        : null

    const numericTarget = explicitNumber
        ? resolveTarget(
            `${explicitNumber}@s.whatsapp.net`,
            participants,
            conn
        )
        : null
    ...
    global.promoteDemoteCache =
        global.promoteDemoteCache || new Map()

    const cacheKey =
        `promote_${m.chat}_${user}`
...
╰━━━━━━━━━⪩`
        ...
        const fallbackPath =
            path.join(__dirname, '../../media/fallback.png')
