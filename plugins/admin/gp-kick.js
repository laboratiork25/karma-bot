...
async function handler(m, { isBotAdmin, isOwner, text, command, conn, setKickReason }) {
  ...
  const kickCaption = isSimulation
    ? `🎭 *Simulazione kick*\n\n@${mention.split('@')[0]} sarebbe stato rimosso da @${m.sender.split('@')[0]}${reasonText}`
    : `👢 @${mention.split('@')[0]} è stato rimosso da @${m.sender.split('@')[0]}${reasonText}`
  ...
  if (!isSimulation) {
    setKickReason?.(m.chat, mention, {
      type: 'kickcmd',
      reason: 'Espulsione tramite comando kick',
      details: cleanReason || 'nessuna motivazione fornita',
      source: 'plugins/admin/kick.js',
      messageId: m.key.id,
      rawText: text || ''
    }, m.sender)
