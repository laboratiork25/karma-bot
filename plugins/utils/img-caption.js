...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  try {
    if (!text) {
      return m.reply(
        `✳️ Esempio:\n${usedPrefix + command} questo è il testo\n\n` +
        `Uso:\n- rispondi a una foto con ${usedPrefix + command} testo\n` +
    ...
    const tempFilePath = path.join(os.tmpdir(), `caption_${Date.now()}.jpg`)
    ...
    const apiUrl =
      `https://api.popcat.xyz/v2/caption?image=${encodeURIComponent(imageUrl)}` +
      `&text=${encodeURIComponent(text)}` +
      `&bottom=false&dark=true&fontsize=67`
...
handler.help = ['caption <testo>', 'variante <testo>']
handler.tags = ['fun']
handler.command = /^(caption|scritta)$/i
