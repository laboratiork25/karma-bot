...
function getUploadOption(conn) {
  return typeof conn.waUploadToServer === 'function' ? { upload: conn.waUploadToServer } : {};
}
...
async function prepareInteractiveMedia(conn, buffer, options = {}) {
  ...
  if (/^https?:\/\//i.test(buffer)) {
    try {
      ...
      if (/^image\//i.test(contentType)) {
        img = await prepareWAMessageMedia({ image: { url: buffer } }, { ...getUploadOption(conn), ...options });
      } else if (/^video\//i.test(contentType)) {
        video = await prepareWAMessageMedia({ video: { url: buffer } }, { ...getUploadOption(conn), ...options });
      } else if (contentType) {
  ...
  try {
    ...
    if (/^image\//i.test(type.mime)) {
      img = await prepareWAMessageMedia({ image: type.data }, { ...getUploadOption(conn), ...options });
    } else if (/^video\//i.test(type.mime)) {
      video = await prepareWAMessageMedia({ video: type.data }, { ...getUploadOption(conn), ...options });
    }
  } catch (error) {
        ...
        for (const media of medias) {
            ...
            if (/^https?:\/\//i.test(data.url)) {
                try {
                    ...
                    if (/^image\//i.test(contentType)) {
                        img = await prepareWAMessageMedia({ image: { url: data.url } }, { ...getUploadOption(conn) });
                    } else if (/^video\//i.test(contentType)) {
                        video = await prepareWAMessageMedia({ video: { url: data.url } }, { ...getUploadOption(conn) });
                    }
                } catch (error) {
                    console.error("Errore nell'ottenere il tipo MIME:", error);
                }
