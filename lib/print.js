...
${chalk.hex('#FE0041').bold('╰★────★────★────★────★')}`)
    ...
    // Debug per view once e messaggi eliminati
    if (m.msg?.viewOnceMessage) {
      console.log(chalk.cyan('[DEBUG VIEW ONCE]'), 'viewOnceMessage rilevato')
      console.log('Struttura:', JSON.stringify(m.msg.viewOnceMessage, null, 2))
    }
    if (m.msg?.imageMessage?.viewOnce) {
      console.log(chalk.cyan('[DEBUG VIEW ONCE IMAGE]'), 'imageMessage.viewOnce:', m.msg.imageMessage.viewOnce)
    }
    if (m.msg?.videoMessage?.viewOnce) {
      console.log(chalk.cyan('[DEBUG VIEW ONCE VIDEO]'), 'videoMessage.viewOnce:', m.msg.videoMessage.viewOnce)
    }
    if (m.msg?.protocolMessage) {
      console.log(chalk.magenta('[DEBUG PROTOCOL MESSAGE]'), 'tipo:', m.msg.protocolMessage.type)
      console.log('Struttura:', JSON.stringify(m.msg.protocolMessage, null, 2))
    }
